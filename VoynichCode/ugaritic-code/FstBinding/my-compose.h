...
#ifndef FST_LIB_MYCOMPOSE_H__
#define FST_LIB_MYCOMPOSE_H__
...
// and state tables.
template <class A>
class MyComposeFstImplBase : public CacheImpl<A> {
 public:
  ...
  MyComposeFstImplBase(const MyFst<A> &fst1,
                       const MyFst<A> &fst2,
  ...
  MyComposeFstImplBase(const MyComposeFstImplBase<A> &impl)
      : CacheImpl<A>(impl),
  ...
  virtual MyComposeFstImplBase<A> *Copy() = 0;

  virtual ~MyComposeFstImplBase() {
    delete fst1_;
    delete fst2_;
  }
 ...
 protected:
  ...
  public: const MyFst<A> *fst1_;            // first input Fst
  public: const MyFst<A> *fst2_;            // second input Fst
};
...
class MyComposeFstImpl : public MyComposeFstImplBase<A> {
  typedef typename A::StateId StateId;
  typedef typename A::Label   Label;
  typedef typename A::Weight  Weight;
  ...
  using FstImpl<A>::SetType;
  using FstImpl<A>::SetProperties;
  using MyComposeFstImplBase<A> ::fst1_;
  using MyComposeFstImplBase<A> ::fst2_;

  typedef ComposeStateTuple<StateId, FilterState> StateTuple;


 public:
  MyComposeFstImpl(const MyFst<A> &fst1,
                   const MyFst<A> &fst2,
                   const ComposeFstOptions<A, M, F, T> &opts)
      : MyComposeFstImplBase<A>(fst1, fst2, opts),
  ...
  MyComposeFstImpl(const MyComposeFstImpl<A, M, F, T> &impl)
      : MyComposeFstImplBase<A>(impl),
  ...
  virtual MyComposeFstImpl<A, M, F, T> *Copy() {
    return new MyComposeFstImpl<A, M, F, T>(*this);
  }
  ...
  StateTuple GetTuple(StateId s) {return state_table_->Tuple(s);}
 ...
 private:
  ...
  // the one to match on.
  void OrderedExpand(StateId s, const MyFst<A> *fsta, StateId sa,
                     const MyFst<A> *fstb, StateId sb,
    ...
    for (ArcIterator< MyFst<A> > iterb(*fstb, sb); !iterb.Done(); iterb.Next())
  ...
  void operator=(const MyComposeFstImpl<A, M, F, T> &);  // disallow
};
...
template <class A>
class MyComposeFst : public MyFst<A> {
 public:
  friend class ArcIterator< MyComposeFst<A> >;
  friend class CacheStateIterator< MyComposeFst<A> >;
  friend class StateIterator< MyComposeFst<A> >;
  friend class CacheArcIterator< MyComposeFst<A> >;
  ...
  MyComposeFst(const MyFst<A> &fst1, const MyFst<A> &fst2)
      : impl_(Init(fst1, fst2, ComposeFstOptions<A>())) {}

  template <class M, class F, class T>
  MyComposeFst(const MyFst<A> &fst1,
             const MyFst<A> &fst2,
             const ComposeFstOptions<A, M, F, T> &opts)
      : impl_(Init(fst1, fst2, opts)) {}

  MyComposeFst(const MyComposeFst<A> &fst, bool reset = false) {
    if (reset) {
  ...
  virtual ~MyComposeFst() { if (!impl_->DecrRefCount()) delete impl_;  }
  ...
  virtual MyComposeFst<A> *Copy(bool reset = false) const {
    return new MyComposeFst<A>(*this, reset);
  }
 ...
 protected:
  MyComposeFst() : impl_(0) {}

  MyComposeFstImplBase<A> *Impl() { return impl_; }

  void SetImpl(MyComposeFstImplBase<A> *impl) { impl_ = impl; }

  template <class M, class F, class T>
  static MyComposeFstImpl<A, M, F, T> *Init(
      const MyFst<A> &fst1,
      const MyFst<A> &fst2,
      const ComposeFstOptions<A, M, F, T> &opts) {
    ...
    return new MyComposeFstImpl<A, M, F, T>(fst1, fst2, opts);
  }

 public:
  MyComposeFstImplBase<A> *impl_;
  ...
  MyFst<A> *ToMyFst() {
    ...
    MyFst<A> *out = new MyFst<A>();
    for (StateIterator< MyComposeFst<A> > siter(*this);
          !siter.Done();
          siter.Next()) {
      ...
      StateId s1 = tuple.state_id1;
      StateId s2 = tuple.state_id2;
      const MyFst<A>* fst1 = impl_->fst1_;
      out->impl_->GetState(s)->edit = fst1->impl_->GetState(s1)->edit;
      out->SetFinal(s, Final(s));
      for (ArcIterator< MyComposeFst<A> > aiter(*this, s);
            !aiter.Done();
            aiter.Next()) {
 ...
 private:
  void operator=(const MyComposeFst &fst);  // disallow
};


// Specialization for ComposeFst.
template<class A>
class StateIterator< MyComposeFst<A> >
    : public CacheStateIterator< MyComposeFst<A> > {
 public:
  explicit StateIterator(const MyComposeFst<A> &fst)
      : CacheStateIterator< MyComposeFst<A> >(fst) {}

  typedef SequenceComposeFilter<A> F;
  typedef typename A::StateId StateId;
  typedef typename F::FilterState FilterState;
  typedef Matcher<MyFst<A> > M;
  typedef GenericComposeStateTable<A, FilterState> T;
  typedef ComposeStateTuple<StateId, FilterState> StateTuple;  
  
  StateTuple GetTuple() { 
    MyComposeFstImpl<A, M, F, T>* impl = (MyComposeFstImpl<A, M, F, T>*) this->fst_.impl_;
    return impl->GetTuple(this->s_); 
  }
...
// Specialization for ComposeFst.
template <class A>
class ArcIterator< MyComposeFst<A> >
    : public CacheArcIterator< MyComposeFst<A> > {
 public:
  typedef typename A::StateId StateId;

  ArcIterator(const MyComposeFst<A> &fst, StateId s)
      : CacheArcIterator< MyComposeFst<A> >(fst, s) {
...
void MyComposeFst<A>::InitStateIterator(StateIteratorData<A> *data) const {
  data->base = new StateIterator< MyComposeFst<A> >(*this);
}
...
//     introduce non-coaccessible states and transitions.
template<class Arc>
void MyCompose(const MyFst<Arc> &ifst1, const MyFst<Arc> &ifst2,
             MutableFst<Arc> *ofst,
             const ComposeOptions &opts = ComposeOptions()) {
  ComposeFstOptions<Arc> nopts;
  nopts.gc_limit = 0;  // Cache only the last state for fastest copy.
  *ofst = MyComposeFst<Arc>(ifst1, ifst2, nopts);
  if (opts.connect)
    Connect(ofst);
}
