...
#ifndef FST_LIB_MYINTERSECT_H__
#define FST_LIB_MYINTERSECT_H__

#include <algorithm>
#include <vector>

#include <fst/cache.h>
#include "my-compose.h"
...
template <class A>
class MyIntersectFst : public MyComposeFst<A> {
 public:
  using MyComposeFst<A>::Impl;
  
  typedef A Arc;
  typedef typename A::Weight Weight;
  typedef typename A::StateId StateId;

  MyIntersectFst(const MyFst<A> &fst1, const MyFst<A> &fst2)
      : MyComposeFst<A>(fst1, fst2) {
  ...
  MyIntersectFst(const MyIntersectFst<A> &fst, bool reset = false) :
      MyComposeFst<A>(fst, reset) {}

  virtual MyIntersectFst<A> *Copy(bool reset = false) const {
    return new MyIntersectFst<A>(*this, reset);
  }
};
...
template<class A>
class StateIterator< MyIntersectFst<A> >
    : public StateIterator< MyComposeFst<A> > {
 public:
  explicit StateIterator(const MyIntersectFst<A> &fst)
      : StateIterator< MyComposeFst<A> >(fst) {}
};


// Specialization for IntersectFst.
template<class A>
class ArcIterator< MyIntersectFst<A> >
    : public ArcIterator< MyComposeFst<A> > {
 public:
   typedef typename A::StateId StateId;
   
  ArcIterator(const MyIntersectFst<A> &fst, StateId s)
      : ArcIterator< MyComposeFst<A> >(fst, s) {}
};
...
template<class Arc>
void MyIntersect(const MyFst<Arc> &ifst1, const MyFst<Arc> &ifst2,
             MutableFst<Arc> *ofst,
             const IntersectOptions &opts = IntersectOptions()) {
  IntersectFstOptions<Arc> nopts;
  nopts.gc_limit = 0;  // Cache only the last state for fastest copy.
  *ofst = MyIntersectFst<Arc>(ifst1, ifst2, nopts);
  if (opts.connect)
    Connect(ofst);
}
